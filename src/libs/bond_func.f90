subroutine get_dist (na, dist, atom1, atom2, xa, cell, coord)
    ! Get the distance between two atoms, considering the pbc of cell
    ! To be honest, this part is confusing. 
    implicit None
    ...
    double precision, dimension(3, 2) :: xa2, xacna
    double precision, dimension(3, 3) :: cell, celli
    double precision :: str(3)
    double precision, optional, dimension(3) :: coord

    call reciprocal_latt (cell, celli)
    ...
    do i1 = -1, 1
      do i2 = -1, 1
        do i3 = -1, 1
          ! Based on the atom1, check all posible atoms of neighbouring cell
          do j = 1,3
            xa2(j,2) = cell(j,1) * (xacna(1,2)+i1) + &
                       cell(j,2) * (xacna(2,2)+i2) + &
                       cell(j,3) * (xacna(3,2)+i3)
          end do
...
subroutine get_conn (na, iza, xa, conn, cell) 
  ...
  integer :: i, j
  double precision :: dist, co1, co2, totbond
  ...
  !print *, "    start to get the connection matrix"
  conn = 0
  ! conn is a symmetry matrix, only calculate the upper triangle
  do i = 1, na
    do j = 1, i-1
      ...
      ! only consider the bond between two atoms ,which less than 18
      ! and the critical distance is less than totbond + 0.2
      !print *, dist-totbond
  ...
  ! do extra judgement for the bond between H and O
  do i = 1, na
    if (iza(i) == 1) then
      minOHbond = -1
      minOHdist = 999999d0
      ...
      do j = 1, na
        if (iza(j)==8 .and. conn(i, j)==1) then
          ! check the connection between H-O
          ! get the number of O-H bond, and minimum bond of H-O, if exists
          counter = counter + 1
          call get_dist(na, dist, i, j, xa, cell)
      ...
      ! if O-H bond number > 1, only consider the minumum bond
      if (counter >1) then
        do j = 1, na
      ...
      ! if O-H bond number = , while the minmumal distance > 1.147, then remove the bond
...
subroutine reciprocal_latt (A, B)
  ! The following section depends on array form, that is \vec a = (A(1,1), A(2,1), A(3,1))
  ! otherwise, a = (A(1,1), A(1,2), A(1,3)), then CI = C/(A(I,1)*B(I,1)+A(I,2)*B(I,2)+A(I,3)*B(I,3))
    implicit None
    integer :: I
...
end subroutine reciprocal_latt

subroutine cross_3d (a, b)
  ! cross_3d computes the cross product of two vectors in 3D space.
  !
  ! Definition:
  !   
  !   The cross product can be regarded as the determinant of the symbolic matrix:
  !           
  !                   | i  j  k |
  !               det | x1 y1 z1|
  !                   | x2 y2 z2|
...
subroutine radius (atom, atomicRadius)
    implicit None
    integer :: atom
...
subroutine get_all_bondmatrix (na, iza, xa, conn, cell) 
  implicit None
  integer :: na
  integer, dimension(na) ::iza
  double precision, dimension(3, na) :: xa
...
end subroutine get_all_bondmatrix