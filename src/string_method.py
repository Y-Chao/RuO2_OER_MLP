...
class String2D:
    ...
    def compute_mep(
        ...
        for tstep in tqdm(range(1, maxsteps + 1)):
            ...
            if integrator == "forward_euler":
                old_string[:] = string
                string = self.step_euler(string, dt, interp_method, flexible=flexible)
            else:
    ...
    def step_euler(self, string, dt, interp_method="linear", flexible=True):
        """
        ...
        string_grad_x = griddata(self.grid, self.gradX.ravel(), string, method=interp_method)
        string_grad_y = griddata(self.grid, self.gradY.ravel(), string, method=interp_method)
